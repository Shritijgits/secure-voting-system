...
// ============================================================
// VOTER DATABASE
// To add face: paste faceDescriptor array, set faceRegistered: true
// ============================================================
...
function checkDuplicateFace(incomingDescriptor, currentVoterId) {
  for (const [voterId, voter] of Object.entries(VOTER_DATABASE)) {
    if (voterId === currentVoterId) continue;
    ...
    if (dist < 0.5) {
      console.log(`DUPLICATE FACE: ${currentVoterId} matches ${voterId} dist=${dist.toFixed(3)}`);
...
function verifyOwnFace(incomingDescriptor, storedDescriptor) {
  ...
  console.log(`Face match distance: ${dist.toFixed(4)} (pass if < 0.5)`);
  return dist < 0.5;
}
...
function sendSMS(mobile, otp) {
  ...
  if (FAST2SMS_API_KEY === 'YOUR_FAST2SMS_API_KEY') {
    console.log(`No SMS key set. OTP in terminal only.`);
    return;
  }
  ...
  const req = https.request(options, (res) => {
    ...
    res.on('end', () => {
      ...
      } catch { console.log('SMS response:', data); }
    });
  });
...
app.post('/api/verify-face', (req, res) => {
  ...
  if (isDuplicate) {
    ...
    return res.status(403).json({ success: false, message: 'This face belongs to a different Voter ID. Blocked!' });
  }
...
app.post('/api/cast-vote', (req, res) => {
  ...
  nominee.votes++;
  voter.hasVoted = true;
  const blockHash = crypto.createHash('sha256').update(session.voterId + nomineeId + Date.now().toString()).digest('hex');
  VOTE_LOG.push({ voterId: session.voterId, voterName: voter.name, nomineeName: nominee.name, timestamp: new Date().toISOString(), blockHash });
  delete FACE_SESSIONS[sessionId];
  return res.json({
    success: true,
...
app.get('/api/admin/results', (req, res) => {
  ...
  return res.json({ success: true, totalVotes: VOTE_LOG.length, results: NOMINEES.map(({ name, votes }) => ({ name, votes })), voteLog: VOTE_LOG });
});


const PORT = process.env.PORT || 3000;
app.listen(PORT, '0.0.0.0', () => {
  console.log(`\nSecureVote running on port ${PORT}`);
